...
2. [Prerequisites](#2-prerequisites)
    ...
    - 2.2 [Setting Up Nexapush CLI](#22-setting-up-nexapush-cli)
3. [Migrating from Appcenter to Nexapush](#3-migrating-from-appcenter-to-nexapush)
    - 3.1 [Logging In](#31-logging-in)
    - 3.2 [Migrating Applications and Deployments](#32-migrating-applications-and-deployments)
4. [Setting Up Nexapush from Scratch](#4-setting-up-nexapush-from-scratch)
...
Welcome! This guide will help you migrate your Over-the-Air (OTA) updates from Appcenter to Nexapush. 
With Nexapush's Command Line Interface (CLI), you can easily push updates directly to your users' devices 
without needing to go through app stores.

**What You'll Learn:**
- How to install the Nexapush CLI
- Migrating your existing OTA updates from Appcenter
- Setting up and managing your applications and deployments in Nexapush
- Updating your React Native app to use Nexapush for OTA updates
...
- **Nexapush Account:**
    - Sign up at [Nexapush](https://nexapush.org/) using GitHub or Google for authentication.
...
### 2.2 Setting Up Nexapush CLI
Similarly to Appcenter Nexapush manages the most of CodePush's functionality using CLI.

To install it run the following command:

**Install Nexapush CLI:**
```shell
   npm install -g @nexapush/code-push-cli
```

**Update Nexapush CLI to the Latest Version:**
```shell
   npm update -g @nexapush/code-push-cli
```

Upon installation or update, you can check the version of the Nexapush CLI and the list of available commands by running the following command:
...
which will display the version of the Nexapush CLI you have installed with available commands.
...
Usage: nexapush <command>

Commands:
  nexapush access-key     View and manage the access keys associated with your account
  nexapush app            View and manage your CodePush apps
  nexapush collaborator   View and manage app collaborators
  nexapush debug          View the CodePush debug logs for a running app
  nexapush deployment     View and manage your app deployments
  nexapush login          Authenticate with the CodePush server in order to begin managing your apps
  nexapush logout         Log out of the current session
  nexapush patch          Update the metadata for an existing release
  nexapush promote        Promote the latest release from one app deployment to another
  nexapush register       Register a new CodePush account
  nexapush release        Release an update to an app deployment
  nexapush release-react  Release a React Native update to an app deployment
  nexapush rollback       Rollback the latest release for an app deployment
  nexapush session        View and manage the current login sessions associated with your account
  nexapush whoami         Display the account info for the current login session
...
```
 ...
 ## 3. Migrating from Appcenter to Nexapush

You need to execute steps in this section if you have been using Appcenter for OTA updates and wish to migrate to Nexapush.

If you did not use Appcenter OTA updates before and just wish to apply
OTA on top of Nexapush for your app feel free to skip this section and go to section [4](#4-setting-up-nexapush-from-scratch).
...
Similarly, in a separate window login to Nexapush CLI using the following command:
...
**2. Create Corresponding Nexapush Apps:**

Remove the username prefix when creating apps in Nexapush.

```shell
  nexapush app add rn2_ios
  nexapush app add rn2_android
```
...
Add the same deployments in Nexapush with identical keys

```shell
nexapush deployment add rn2_ios appcenter_Staging -k <Staging_Key>
nexapush deployment add rn2_ios appcenter_Production -k <Production_Key>
```
...
nexapush deployment add rn2_android appcenter_Staging -k <Staging_Key>
nexapush deployment add rn2_android appcenter_Production -k <Production_Key>
```

Repeat for all other deployment you have in Appcenter and wish to continue to use in Nexapush.

## 4. Setting Up Nexapush from Scratch

If you're new to Nexapush and want to set up OTA updates for your app, follow these steps.
...
```shell
nexapush app add myAmazingApp_ios
```

#### For Android:

```shell
nexapush app add myAmazingApp_android
```
...
```shell
nexapush deployment add myAmazingApp_android Development
```

List All Deployments:

```shell
nexapush deployment ls myAmazingApp_android -k
```

## 5. Updating Your React Native App

After setting up Nexapush, update your React Native application to point to the Nexapush server.
...
<string>https://api.nexapush.org</string>
```
...
<string moduleConfig="true" name="CodePushServerUrl">https://api.nexapush.org</string>
```
...
```shell
nexapush release-react rn2_ios ios -d Staging
```

**What Happens:**
- Bundles your JavaScript code.
- Uploads the bundle and assets to Nexapush.
...
```shell
nexapush release-react rn2_android android -d appcenter-Staging
```
...
```shell
nexapush release-react -h
```

## 7. Summary and Resources


Migrating your OTA updates from Appcenter to Nexapush is straightforward with our CLI tools. 
Whether you're migrating existing applications or setting up new ones,
Nexapush offers a seamless experience with familiar commands and robust features.


**Quick Command Reference:**

| Appcenter                        | Nexapush                |                                                                     Comment                                                                      | 
|:---------------------------------|:------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------:|
| `appcenter login`                 | `nexapush login`         |                                                                      Log in                                                                      |
| `appcenter codepush deployment`   | `nexapush deployment`    |                                                       View and manage your app deployments                                                       |
| `appcenter apps`                  | `nexapush app`           |                                                            View and manage your apps                                                             |
| `appcenter codepush patch`        | `nexapush patch`         |                                               Update the metadata for an existing CodePush release                                               |
| `appcenter codepush promote`      | `nexapush promote`       | Create a new release for the destination deployment, which includes the exact code and metadata from the latest release of the source deployment |
| `appcenter codepush release-react`| `nexapush release-react` |                                                Release a React Native update to an app deployment                                                |
| `appcenter codepush rollback`    | `nexapush rollback`       |                                                   Rollback a deployment to a previous release                                                    |
| `appcenter logout`               | `nexapush logout`         |                                                                     Log out                                                                      |
...
**Need More Help?**
Contact our support team at [support@nexapush.org](mailto:support@nexapush.org) or visit our [website](https://nexapush.org).
