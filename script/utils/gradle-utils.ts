...
// "nexapush"-prefixed to avoid collisions with project tasks; written in Groovy,
// which Gradle accepts as an init script for projects of either DSL.
const PRINT_VERSION_TASK = "_nexapushPrintVersion";
...
async function evaluateKotlinDslBuildFile(buildFile: string): Promise<GradleVersionFields> {
  ...
  const initScript = path.join(os.tmpdir(), `nexapush-init-${process.pid}.gradle`);
